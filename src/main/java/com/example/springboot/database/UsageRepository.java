...
public interface UsageRepository extends CrudRepository<UsageData, Integer> {
    ...
    @Query(value = "SELECT task_type, element, event FROM temp_storage.usage_data where task_id = :taskid AND event in('click', 'blur', 'change', 'keyup', 'Empty', 'Backspace', 'Delete', 'Typing') group by task_type, element", nativeQuery = true)
